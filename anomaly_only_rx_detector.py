...
class GeneratedAnomalyRXDetector:
    ...
    def _build_ui(self):
        ...
        detect_box = ttk.LabelFrame(left, text="Baseline")
        ...
        ttk.Label(detect_box, text="Threshold").grid(row=0, column=0, sticky="w", padx=8, pady=(6, 2))
        ttk.Entry(detect_box, textvariable=self.threshold_var, width=10).grid(row=0, column=1, sticky="w", pady=(6, 2))
        ttk.Checkbutton(
            detect_box,
            text="Normalise ROI scores",
        ...
        ttk.Button(detect_box, text="Anomaly 검출", command=self._detect).grid(
            row=2, column=0, columnspan=2, sticky="ew", padx=8, pady=(6, 6)
        )
    ...
    def _detect(self):
        ...
        try:
            ...
            roi_mask = self.target_label == ROAD_LABEL
            md_map = self._rx_baseline_roi(self.cube, roi_mask, self.normalise_var.get())
            ...
            self._show_image(self.score_canvas, score_rgb)
            self._show_image(self.detect_canvas, detection_rgb)
            saved_paths = self._save_results(md_map, detection_mask, score_rgb, detection_rgb)
        except Exception as exc:
        ...
        self.info_var.set(
            f"{self.current_base} | Road ROI only | threshold={threshold:.3f} | "
            f"detected={detected_pixels}/{roi_pixels} | max score={max_score:.3f} | saved to {saved_paths['dir']}"
        )
    ...
    def _rx_baseline_roi(self, cube, roi_mask, normalise):
        ...
        if spectra.shape[0] <= spectra.shape[1]:
            raise ValueError(
                f"Not enough ROI pixels for covariance: pixels={spectra.shape[0]}, bands={spectra.shape[1]}"
            )
    ...
    def _save_results(self, md_map, detection_mask, score_rgb, detection_rgb):
        self.result_dir.mkdir(parents=True, exist_ok=True)
        base = self.current_base
        md_path = self.result_dir / f"{base}_baseline_scores.npy"
        binary_path = self.result_dir / f"{base}_baseline_binary.png"
        score_path = self.result_dir / f"{base}_baseline_score_map.png"
        overlay_path = self.result_dir / f"{base}_baseline_overlay.png"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run baseline detection on generated anomaly samples.")
